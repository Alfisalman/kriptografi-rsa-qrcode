    ...
    public function tambah_data()
    {
        ...
        if ($this->input->post('submit', TRUE) == 'Submit') {
            ...
            if ($this->form_validation->run() == TRUE) {
                ...
                if ($simpan) {
                    $last_data = $this->m_pembelian->getLastInsert();
                    // generate qrcode with data = url
                    $url = site_url('show_surat/'.encryptX($last_data['id_pembelian']));
                    $file_name = 'qrcode_'.$last_data['id_pembelian'].'.png';
                    generate($url, $file_name);
                    $this->m_pembelian->update('tbl_pembelian', ['qrcode' => $file_name], ['id_pembelian' => $last_data['id_pembelian']]);
    ...
    public function hapus_data()
    {
        ...
        if ($this->input->is_ajax_request()) {
            ...
            if ($this->form_validation->run() == TRUE) {
                ...
                if ($hapus) {
                    $file_name = 'qrcode_'.$this->input->post('id', TRUE).'.png';
                   unlink('./assets/img_qrcode/'.$file_name); 
            ...
            foreach ($list as $i) {
                ...
                if ($this->session->userdata('level') == 'admin' || $this->session->userdata('UserID') == $i->id_user) :
                    ...
                    $button .= '<a href="' . site_url('edit_pembelian/' . $i->id_pembelian) . '" class="btn btn-warning btn-sm text-white">Edit</a>
                        ...
                        <a type="button" href="'. base_url("cetak_qr/$i->qrcode") .'" style="color: white;" class="btn btn-info btn-sm">Cetak QR</a>';
                ...
                $row[] = $i->fullname;
                $row[] = '<img src="'.base_url().'assets/img_qrcode/'.$i->qrcode.'" width="50">';
